...
import { createAGroup, updateAGroup } from "../../store/group";
...
export default function GroupUpdateForm() {
    ...
    const [groupPurpose, setGroupPurpose] = useState(group ? group.about : '');
  
    const [groupType, setGroupType] = useState(group ? group.type : 'Select One');
  
    const [groupStatus, setGroupStatus] = useState(group ? group.private : 'Select One');
    ...
    const onSubmit = async (e) => {
      ...
      const response = await dispatch(updateAGroup(groupId,updateGroup));
  ...
  return (
    ...
    <form className="groupFormContainer" onSubmit={onSubmit}>
      ...
      <button className='createGroupButton' type="submit">Update Group</button>
    </form>
  </div>
);
}
